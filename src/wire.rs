...
pub fn validate_quote_request(request: &interfaces::QuoteRequest) -> Outcome {
    ...
    push(&mut issues, "organizationName", text(&request.organization_name, 200));
    push(&mut issues, "contactName", text(&request.contact_name, 160));
    push(&mut issues, "contactEmail", email(&request.contact_email));
    push(&mut issues, "website", request.website.as_deref().and_then(website));
    push(&mut issues, "employeeCount", employee_count(request.employee_count));
    push(
        &mut issues,
        "annualRevenueBand",
        request
    ...
    push(&mut issues, "frameworks", set(&request.frameworks, FRAMEWORKS));
    push(
        &mut issues,
        "currentStage",
    ...
    push(
        &mut issues,
        "notes",
        request.notes.as_deref().and_then(|notes| too_long(notes, 5_000)),
    );
    push(
        &mut issues,
        "contextKey",
